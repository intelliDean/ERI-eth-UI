...
const VerifyPage = () => {
  ...
  const verifyAuthenticity = async (cert?: any, sig?: string) => {
    ...
    if (!authenticityRContract || !targetCert || !targetSig) {
    ...
    try {
      setIsLoading(true);
      const result = await authenticityRContract.verifyAuthenticity(targetCert, targetSig);
      ...
      if (isValid) {
        ...
        await checkOwnership(targetCert.uniqueId);
      } else {
  ...
  const checkOwnership = async (itemId: string) => {
    ...
    try {
      const ownership = await ownershipRContract.verifyOwnership(itemId);
  ...
  const claimOwnership = async () => {
    ...
    try {
      ...
      const tx = await authenticitySContract.userClaimOwnership(certificate, signature);
      await tx.wait();
      
      toast.success('Ownership claimed successfully!');
      // Refresh ownership info
      await checkOwnership(certificate.uniqueId);
    } catch (error: any) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              The QR code you scanned doesn't contain valid certificate data.
            </p>
            <Link
              to="/"
                      ...
                      <div>
                        ...
                        {verificationResult.isValid && verificationResult.manufacturerName && (
                          <p className="text-green-700">
                            Manufactured by: {verificationResult.manufacturerName}
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {!account ? (
                  <div className="text-center py-8">
                    ...
                    <button
                      ...
                      className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-8 py-3 rounded-lg font-semibold hover:from-emerald-700 hover:to-emerald-800 transition-all duration-300 shadow-lg hover:shadow-xl"
                    >
                      Connect Wallet
                    </button>
                  </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mb-6">
                      Click below to claim ownership of this verified product.
                    </p>
                    <button
                ...
                {ownershipInfo ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600">
                      This product has not been claimed by any user yet.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-gray-900">
                  {new Date(Number(certificate.date) * 1000).toLocaleDateString()}
                </p>
              </div>
